...
MM.Command.New.execute = function() {
	if (!confirm("確認捨棄目前的心智圖和新增一個新的 ?")) { return; }
	var map = new MM.Map();
	MM.App.setMap(map);
...
MM.Command.Value.execute = function() {
	...
	var newValue = prompt("指定項目值", oldValue);
...
MM.Layout.Graph.Down = MM.Layout.Graph.create("bottom", "graph-bottom", "底部");
MM.Layout.Graph.Up = MM.Layout.Graph.create("top", "graph-top", "頂部");
MM.Layout.Graph.Left = MM.Layout.Graph.create("left", "graph-left", "左邊");
MM.Layout.Graph.Right = MM.Layout.Graph.create("right", "graph-right", "右邊");
...
MM.Layout.Tree.Left = MM.Layout.Tree.create("left", "tree-left", "左邊");
MM.Layout.Tree.Right = MM.Layout.Tree.create("right", "tree-right", "右邊");
...
MM.UI.IO = function() {
    ...
    var ids = ["local", "firebase", "gdrive", "file", "webdav"];
...
MM.UI.IO.prototype.show = function(mode) {
    ...
    this._heading.innerHTML = (mode == "load" ? "載入心智圖" : "儲存心智圖");
...
MM.UI.Backend.show = function(mode) {
    ...
    this._go.innerHTML = (mode == "load" ? "載入" : "儲存");
...
MM.UI.Backend.File.show = function(mode) {
	...
	this._go.innerHTML = (mode == "save" ? "儲存" : "瀏覽");
}
